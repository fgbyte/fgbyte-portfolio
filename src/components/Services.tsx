...
export default function Services({ data, tags }: Props) {
  const [filter, setFilter] = createSignal(new Set<string>())
  const [services, setServices] = createSignal<CollectionEntry<"services">[]>([])

  createEffect(() => {
    setServices(data.filter((entry) =>
      ...
      <div class="col-span-3 sm:col-span-2">
        <div class="flex flex-col">
          <div class="text-sm uppercase mb-2">
            SHOWING {services().length} OF {data.length} SERVICES
          </div>
          <ul class="flex flex-col gap-3">
            {services().map((service) => (
              <li>
                <ArrowCard entry={service} />
              </li>
            ))}
          </ul>
        </div>
